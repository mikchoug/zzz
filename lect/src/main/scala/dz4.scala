...
object CaseSquareFigures extends App with ImplicitForPrint {

  trait Figurs

  case class Circle(radius: Int) extends Figurs

  case class Rectangle(a: Int, b: Int) extends Figurs

  case class Square(a: Int) extends Figurs

  def calculeSquare(F: Figurs): Unit = {
    F match {
      case c: Circle =>
        s"create with name: ${3.14 * c.radius}".print
      case r: Rectangle =>
        s"delete by id:  with ignore warning: ${r.a * r.b}".print
      case s: Square =>
        s"read by id: ${s.a * s.a} with max size:".print

      case _ => "unkown action".print
    }
  }

  calculeSquare(Circle(21))
  calculeSquare(Rectangle(21,40))
  calculeSquare(Square(21))
}

object OptionI extends App with ImplicitForPrint {

  val o1: Option[String] = Option("haha")

  def srch(ss:String ,z: Option[String]) {
  z match
    {
    case Some(s) => if (s==ss) println("the world is found")
    else println("the world was not found")
...
object AgeGroup extends App with ImplicitForPrint {

  val o1: Option[Int] = Option(15)

  def srch1(z: Option[Int]): Int = z
  match {
    case Some(s) => (s)
    case None => 0
  }

val aaa: Int = srch1(o1)
if (aaa<=1) println("новорожденный возраст")
else if (aaa<11) println("детский возраст")
else if (aaa<16) println("подростковый возраст")
else if (aaa<21) println("юношеский возраст")
else if (aaa<60) println("зрелый возраст")
else if (aaa<90) println("старческий возраст")
else println("долгожитель")
}

object Divis extends App {
  def EitherResultOfDiv(a: Int, b:Int): Either[String, Double] = {
    if (b==0) Left("divise by 0")
    else Right(a/b)
  }
  EitherResultOfDiv(4,1) match {
    case Left(s) => println(s"Error: $s")
    case Right(s) => println(s"Answer: $s")
  }
}

object OptionX extends App {
  def ize(a:Int,b:Int): Option[Double] = {
    try {
      Some(a / b)
    }
    catch {
...
object OptionIII extends App {
import scala.util.Try
  def stringSize1(s: String): Try[Int] = Try(s.toInt)
  val a = stringSize1("5")
  println(a)
}

object OptionIIII extends App {
  import scala.util.Try
  import scala.util.Success
  import scala.util.Failure
  def stringSize1111(a:Int,b:Int): Try[Double] = Try(a/b)
  stringSize1111(5,1) match {
    case Success(addr) =>  println(addr)
    case Failure(e) => println("errr")
  }
}